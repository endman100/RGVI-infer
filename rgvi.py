...
class RGVI(nn.Module):
    def __init__(self):
        ...
        self.sdi = SDI.from_pretrained('stabilityai/stable-diffusion-2-inpainting', torch_dtype=torch.float16, cache_dir='../huggingface').to('cuda')
    ...
    def forward(self, imgs, neg_masks, pos_masks, res, prompt):
        ...
        if 1 in masks[k].unique():
            ...
            # make generator for deterministic generation
            ...
            out = self.sdi(prompt=prompt, image=img, mask_image=mask, generator=generator).images[0]
            out = out.resize((x2 - x1, y2 - y1), Image.BICUBIC)
